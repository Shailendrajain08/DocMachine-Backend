...
const uploadImage = (file) => new Promise((resolve, reject) => {
    async function downloadFile(bucketName, fileName) {
        ...
        function getExcelData(url) {
            console.log('url: ', url);
            // excel(url, function (err, data) {
            //   if (err) throw err;
            //   const temp = JSON.stringify(convertToJSON(data));
            //   console.log(temp);
            //   console.log(data);
            //   resolve(temp)
            // });
        ...
        function findInvoices(line, str) {
            ...
            console.log(secondSNo, secondInvoiceNo, secondInvoiceAmount, secondInvoiceCurrency);
        ...
        //     console.log(i++)
        // }, 1000);
        const result = getExcelData(absolute_path_to_pdf);
        // console.log('result: ', result ? length);
        resolve(result);
        // function callback(error, data) { error ? console.error(error) : console.error(data.text_pages); getExcelData(absolute_path_to_pdf) }
        // resolve(publicUrl)
        // }
        console.log(
